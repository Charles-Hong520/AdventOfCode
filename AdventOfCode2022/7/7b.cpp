...
void findmin(int tot, Node* n, int& currmin) {
	if(30000000 - (70000000-tot)<= n->ds && n->ds < currmin){
		currmin = n->ds;
	}
	for(Node* node : n->dirs) findmin(tot,node,currmin);
}
...
int main() {
	...
	sumAtMost106(root);
	int currmin = INT_MAX;
	findmin(tot, root, currmin);
	cout<<currmin<<" "<<lscnt<< " " << dircnt<<endl;
	return 0;	
}
